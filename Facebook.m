 ...
 */
- (void)dealloc {
  for (FBRequest1* _request in _requests) {
 ...
 */
- (FBRequest1*)openUrl:(NSString *)url
  ...
  FBRequest1* _request = [FBRequest1 getRequestWithParams:params
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
  if (context == finishedContext) {
    FBRequest1* _request = (FBRequest1*)object;
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) {
      ...
      NSString *fbAppUrl = [FBRequest1 serializeURL:urlPrefix params:params];
    ...
    if (trySafariAuth && !didOpenOtherApp) {
      ...
      NSString *fbAppUrl = [FBRequest1 serializeURL:loginDialogURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
  }
...
- (void)dialog:(NSString *)action
  ...
  if (action == kLogin) {
    ...
    _fbDialog = [[FBDialog1 alloc] initWithURL:dialogURL params:params delegate:delegate];
  }
